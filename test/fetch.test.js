...
describe("buildIslandPayload", () => {
    it("respects precedence fixed -> payload -> override", () => {
        expect(buildIslandPayload({ email: "payload@example.com", keep: 1 }, { email: "fixed@example.com", team_id: 10 }, { email: "override@example.com" })).toEqual({
...
describe("islandFetch", () => {
    ...
    it("refreshes csrf cache on 422 responses", async () => {
        ...
        await islandFetch("TeamMembers", "add_member", {}, { fetchImpl });
        expect(fetchImpl.mock.calls[1][1]?.headers).toMatchObject({ "x-csrf-token": "new-token" });
    });
});
